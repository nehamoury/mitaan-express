...
const AdPopup = ({ language = 'en' }) => {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            // Check if already shown in this session (optional, commented out for testing "workable" repeatedly)
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <motion.div
                        ...
                        className="relative w-full max-w-lg bg-white dark:bg-[#0F172A] rounded-3xl shadow-2xl overflow-hidden border border-white/20"
                    >
                        {/* Close Button */}
                        <button
                            onClick={handleClose}
                            className="absolute top-4 right-4 z-20 p-2 bg-black/10 dark:bg-white/10 hover:bg-red-600 hover:text-white rounded-full transition-colors text-slate-500 dark:text-gray-400"
                        >
                            <X size={20} />
                        </button>
